...
import com.ingsis.snippet_service.storageConfig.StorageConfig;
...
public class SnippetController {
    ...
    public ResponseEntity<?> createSnippet(@RequestBody RequestSnippetDTO snippet) {
        ...
        if (!saved.getValidationResult().isValid()) {
            return ResponseEntity.unprocessableEntity()
                    .body("Invalid Snippet: " + saved.getValidationResult().getMessage() + "in line:"
                            + saved.getValidationResult().getLine() + ", column:" +
    ...
    public ResponseEntity<?> createSnippetFromFile(
        ...
        if (!saved.getValidationResult().isValid()) {
            return ResponseEntity.unprocessableEntity()
                    .body("Invalid Snippet: " + saved.getValidationResult().getMessage() +
                            " in line:" + saved.getValidationResult().getLine() +
                            ", column:" + saved.getValidationResult().getColumn());
        }
    ...
    //User Story 4
    @PutMapping("/{id}")
    public ResponseEntity<?> updateSnippet(@PathVariable UUID id, @RequestBody RequestSnippetDTO updatedSnippet) {
        Snippet result = snippetService.updateSnippet(id, new Converter().convertToSnippet(updatedSnippet));
        if (!result.getValidationResult().isValid()) {
            return ResponseEntity.unprocessableEntity()
                    .body("Invalid Snippet: " + result.getValidationResult().getMessage() +
                            " in line: " + result.getValidationResult().getLine() +
                            ", column: " + result.getValidationResult().getColumn());
        }
