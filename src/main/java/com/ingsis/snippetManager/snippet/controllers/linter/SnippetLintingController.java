...
public class SnippetLintingController {
    ...
    public ResponseEntity<?> updateLintingRules(
            @AuthenticationPrincipal Jwt jwt,
            @RequestBody List<UpdateLintingDTO> rulesDTO
    ) {
        try {
            String ownerId = getString(jwt);
    ...
    @PostMapping("/create")
    public ResponseEntity<?> createLintingRules(@AuthenticationPrincipal Jwt jwt, @RequestBody List<LintingDTO> rulesDTO) {
        try {
            String ownerId = getString(jwt);
